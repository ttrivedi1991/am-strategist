import { LIVE_BILLINGS, LIVE_META } from "./live";
import type { Account, AMProfile } from "./mock";

export { LIVE_META };

// Overlay live BigQuery billing series (from `npm run refresh`) onto the
// curated account records. Accounts without an AGID match (e.g. churned
// partners no longer billing) keep their static history.
export function withLiveBillings(accounts: Account[]): Account[] {
  return accounts.map(a => {
    const live = a.agid ? LIVE_BILLINGS[a.agid] : undefined;
    if (!live) return a;
    return { ...a, revenueHistory: live.months, mtdBilling: live.mtd ?? undefined };
  });
}
