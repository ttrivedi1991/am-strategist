...
## Gemini
- Model selection lives in `src/lib/gemini.ts` (`ensureGeminiModel`): version floor 2.5, availability verified daily against the live API, auto-discovers the newest stable flash model when Google retires one. NEVER hardcode a Gemini model string in a fetch URL.
- API key: Firestore `meta/config.geminiApiKey` (GCP key "ttrivedi-am-strategist-gemini" on vendasta-citizen-developers). Re-seed with `GEMINI_API_KEY=... npx tsx scripts/seed-firestore.ts`.
